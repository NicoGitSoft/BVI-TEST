...
plt.axhline(y=2, color='k', linestyle="--" , linewidth=1, alpha=0.5, label=r'Umbral de proximidad de obstáculos')

#plt.plot(times, d, label=r'detected')
#plt.axhline(y=DELTA, color='k', linestyle="--" , linewidth=1, alpha=0.5, label=r'Umbral de detección')
plt.plot([], [], ' ',color='k', label=r'$\eighthnote$ Alerta de obstáculo')
plt.title(r"Alertas de proximidad de obstáculos")
plt.xlabel(r'Tiempo [s]')
plt.ylabel(r'Distancia [m]')
plt.legend(loc='upper right')
plt.xlim(times[0], times[-1])
plt.savefig('Obstacle proximity alerts - Paper.svg', format='svg', dpi=1200)
plt.savefig('Obstacle proximity alerts - Paper.png', format='png', dpi=1200)
plt.savefig('Obstacle proximity alerts - Paper.pdf', format='pdf', dpi=1200)
plt.show()